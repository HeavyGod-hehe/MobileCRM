...
**Generated:** 2026-07-22 05:45:39
**Database:** `/tmp/crm_stress_bug17.db` (0.4 MB)
...
- [PASS] **Purchases/expenses just after local midnight land in today's reports, not UTC-yesterday's** - OK (685ms)
- [PASS] **purchase_invoice_counter persists; unknown setting keys raise instead of vanishing** - OK (684ms)
...
- [PASS] **Concurrent double-sell on the same phone is rejected, not double-posted** - OK (900ms)
...
- [PASS] **Negative cash guard also blocks a bank deposit that would overdraw the drawer** - OK (690ms)
- [PASS] **Reversing one side investment doesn't touch the partner's other top-ups** - OK (682ms)
...
- [PASS] **Forgot-password OTP flow end to end (SMTP transport mocked, real app logic)** - OK (2774ms)
- [PASS] **OTP verification locks out after too many wrong attempts** - OK (685ms)
- [PASS] **OTP expires after 10 minutes** - OK (681ms)
- [PASS] **Restore only touches the requesting user's rows, others untouched** - OK (1396ms)
...
- [PASS] **Restore succeeds cleanly with a concurrent open connection (no corruption)** - OK (724ms)
