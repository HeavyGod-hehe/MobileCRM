...
- [PASS] **Credit = shop received money, debit = shop gave money or billed new debt** - OK (207ms)
- [PASS] **Pre-fix account_entries rows are flipped exactly once by the migration** - OK (277ms)
- [PASS] **Purchase + udhar ledger sync** - OK (2ms)
- [PASS] **Borrow phone ledger sync** - OK (2ms)
- [PASS] **Sale + receivable ledger sync** - OK (3ms)
- [PASS] **Sell phone with no IMEI at all (direct + bulk)** - OK (4ms)
...
- [PASS] **Signup backup runs in the background and still actually completes** - OK (207ms)
- [PASS] **Expense Summary includes phone, fixed, AND account-based expenses** - OK (205ms)
- [PASS] **export_all_data() includes every user-owned table (invoices, JVs, side investments, returns, ledger links)** - OK (201ms)
...
- [PASS] **Close the Month resets only the Overview tile, not Month Report/dashboard history** - OK (2409ms)
- [PASS] **Sale return flow** - OK (4ms)
- [PASS] **Purchase return refund is capped at what was actually paid** - OK (8ms)
- [PASS] **Sale return refund is capped at what the customer actually paid** - OK (7ms)
- [PASS] **Purchases/expenses just after local midnight land in today's reports, not UTC-yesterday's** - OK (204ms)
- [PASS] **customer_recovery_analysis() oldest_outstanding_date is local, not raw UTC** - OK (197ms)
- [PASS] **purchase_invoice_counter persists; unknown setting keys raise instead of vanishing** - OK (199ms)
...
- [PASS] **Mixed-activity ledger reconciliation (220 randomized transactions)** - OK (270ms)
...
- [PASS] **Concurrent double-delete of the same phone is idempotent (no crash, no double-reversal)** - OK (11ms)
...
- [PASS] **Invoice dedupe migration repairs pre-existing duplicates and restores uniqueness** - OK (3ms)
- [PASS] **Negative cash guard blocks by default, allows through with force=True** - OK (206ms)
- [PASS] **Negative bank guard blocks a withdrawal by default, allows through with force=True** - OK (207ms)
- [PASS] **Negative cash guard also blocks a bank deposit that would overdraw the drawer** - OK (202ms)
- [PASS] **Side investment never touches Cash/Bank/partner capital; reversal is per-event** - OK (204ms)
- [PASS] **Device CRUD is isolated from Cash/Bank/Total Investment/Expected Liquid** - OK (199ms)
- [PASS] **Reversing a legacy (pre money-model) side investment cleans up its old cash/capital effect** - OK (204ms)
- [PASS] **Side investment reversal refuses ambiguous pre-fix legacy data instead of guessing** - OK (202ms)
- [PASS] **License activation is hardware-bound; reuse on another machine fails** - OK (2ms)
- [PASS] **Forgot-password OTP flow end to end (SMTP transport mocked, real app logic)** - OK (821ms)
- [PASS] **OTP verification locks out after too many wrong attempts** - OK (203ms)
- [PASS] **Hardware-ID password reset: wrong code/username rejected, valid code changes password** - OK (1001ms)
- [PASS] **Hardware-ID password reset locks out after too many wrong codes** - OK (202ms)
- [PASS] **OTP expires after 10 minutes** - OK (205ms)
- [PASS] **Restore only touches the requesting user's rows, others untouched** - OK (425ms)
- [PASS] **Restore rejects path traversal and another user's backup file** - OK (11ms)
- [PASS] **Reset CRM wipes only the requesting user's data, keeps login intact, other users untouched** - OK (415ms)
- [PASS] **Reset CRM succeeds (no FOREIGN KEY constraint failure) on a phone bought+sold through the bank** - OK (208ms)
...
- [PASS] **Reset CRM clears the user's undo history so it can't resurrect wiped data** - OK (223ms)
- [PASS] **Concurrent backups (racing threads) never collide on the same destination filename** - OK (215ms)
- [PASS] **Backup taken pre-migration restores and cleanly re-migrates after a schema bump** - OK (235ms)
- [PASS] **Restore succeeds cleanly with a concurrent open connection (no corruption)** - OK (230ms)
...
- [PASS] **Signup/auto backup does not deadlock the database (self-.backup() regression)** - OK (208ms)
- [PASS] **Opening-stock phones create ZERO cash/bank ledger rows** - OK (204ms)
- [PASS] **Completed wizard on a fresh user drives liquidity_gap to ~0** - OK (205ms)
- [PASS] **Fixed/overhead expenses never open a false Hisaab mein Farq gap** - OK (204ms)
- [PASS] **Pay-later purchase leaves cash/bank untouched until a payment is recorded** - OK (203ms)
- [PASS] **Selling an opening-stock phone posts the sale ledger only, profit computes normally** - OK (200ms)
- [PASS] **Reclassifying an opening-stock phone away from 'opening' on edit reproduces the reported 77k liquidity gap** - OK (206ms)
- [PASS] **Editing an opening-stock phone while keeping acquisition_type='opening' keeps liquidity_gap at 0** - OK (201ms)
- [PASS] **Existing users with no setup_completed row are grandfathered past the wizard** - OK (206ms)
- [PASS] **Month summary of a seeded test month matches hand-computed numbers** - OK (199ms)
